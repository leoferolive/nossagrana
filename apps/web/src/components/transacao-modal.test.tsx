...
describe('TransacaoModal', () => {
  it('não renderiza quando fechado', () => {
    render(<TransacaoModal familiaId="" open={false} onClose={vi.fn()} onSubmit={vi.fn()} />);
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument();
  });

  it('renderiza campos quando aberto', () => {
    useCategoriaStore.setState({ categorias: CATEGORIAS, carregando: false, erro: null });
    render(<TransacaoModal familiaId="f1" open={true} onClose={vi.fn()} onSubmit={vi.fn()} />);
  ...
  it('chama onClose ao clicar em cancelar', () => {
    const onClose = vi.fn();
    render(<TransacaoModal familiaId="f1" open={true} onClose={onClose} onSubmit={vi.fn()} />);
